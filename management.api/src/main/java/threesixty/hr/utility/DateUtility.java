...
public final class DateUtility {
	...
	private static final List<String> FRY_SAT_WEEKEND_DAYS_COUNTRIES = Arrays.asList(new String[] { "AE", "DZ", "BH",
			"BD", "EG", "IQ", "IL", "JO", "KW", "LY", "MV", "MR", "OM", "PS", "QA", "SA", "SD", "SY", "YE" });
...
//	/**
//	 * format time with {@value DateFormat#SHORT}, {@value DateFormat#SHORT}
//	 * patterns
//	 */
...
//		}
//		Locale loc = NlsLocale.get();
//		return BEANS.get(DateFormatProvider.class).getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT, loc)
//				.format(d);
//	}
...
//	/**
//	 * Creates a {@link SimpleDateFormat} to parse the first argument according to
//	 * the provided pattern. Disables lenient behavior of {@link SimpleDateFormat}.
//	 */
...
//			// default for SimpleDateFormat is a lenient behavior, e.g. 13.13.13,
//			// 952.1238.2010, 12.01.191;ABC would also be valid dates
...
//	/**
//	 * Returns <code>true</code> if, and only if the given String is a valid date
...
//	}
	 ...
	 *
	 * @param count days is truncated to second and can be negative
	...
	/**
	 * truncate the date to week (does not depend on locale, monday is always the
	 * first day in a week)
	 *
	 * @see "ISO 8601"
	 */
	...
	/**
	 * truncate the date to quarter year (i.e. jan 1, apr 1, jul 1 or oct 1 of the
	 * given year)
	 *
	 * @since 4.2
	 */
	 ...
	 *
	 * @param c               Calendar to truncate
	 * @param adjustIncrement -1 to back in time, +1 to go forward
	 */
	...
	/**
	 * truncate the calendar to half year (i.e. jan 1, apr 1, jul 1 or oct 1 of the
	 * given year)
	 *
	 * @since 4.2
	 */
	...
	/**
	 * <b>Timestamp and Date:</b> {@link Timestamp#equals(Object)} is not symmetric.
	 * E.g. when comparing a Date d and Timestamp t, d.equals(t) may return true
	 * while t.equals(d) will always return false. This is not "expected" and
	 * inconvenient when performing operations like sorting on collections
	 * containing both Dates and Timestamps. Therefore, this method handles
	 * <code>java.sql.Timestamp</code> specifically to provide a symmetric
	 * implementation of the equivalence comparison.
	 * <p>
	 * <code>java.sql.Timestamp</code> is a subclass of <code>java.util.Date</code>,
	 * which additionally allows to specify fractional seconds to a precision of
	 * nanoseconds. This method returns <code>true</code>, if and only if both
	 * arguments of Type <code>java.util.Date</code> or
	 * <code>java.sql.Timestamp</code> represent the same point in time at full
	 * precision.
	 */
	...
	/**
	 * Both parameters must be non-null. Implementation of {@code a.compareTo(b)}
	 * that works for Timestamp and Date.
	 * <p>
	 * <b>Timestamp and Date:</b> {@link Timestamp#compareTo(Date)} is not
	 * symmetric. E.g. when comparing a Date d and Timestamp t, d.compareTo(t) may
	 * return true while t.compareTo(d) returns false. This is not "expected" and
	 * inconvenient when performing operations like sorting on collections
	 * containing both Dates and Timestamps. Therefore, this method handles
	 * <code>java.sql.Timestamp</code> specifically to provide a symmetric
	 * implementation of the comparison operation.
	 * <p>
	 * On Java 8 and below, {@link Date#compareTo} returns erroneous results when
	 * the argument is a Timestamp, due to ignoring the fractional seconds part of
	 * the Timestamp (see JDK-8135055). Due to this and in order to retain full
	 * precision, {@link Timestamp#compareTo} must be called. This method returns
	 * <code>0</code> if and only if both arguments of Type
	 * <code>java.util.Date</code> or <code>java.sql.Timestamp</code> represent the
	 * same point in time at full precision.
	 * <p>
	 * Not null safe, see {@link ObjectUtility#compareTo} for a null-safe
	...
	public static int compareTo(Date a, Date b) {
		if (b instanceof java.sql.Timestamp && !(a instanceof java.sql.Timestamp)) {
			...
			// note: could also call use -1 * b.compareTo(a) here, but that equivalence is
			// not specified and a new Timestamp object would be created regardless (in
			// Timestamp.compareTo)
...
//	}
	...
	/**
	 * combine a date (yy/mm/dd) and a time (hh:mm:ss) into a combined
	 * timestamp-date containing both date and time.
	 */
	...
	/**
	 * @param d a value in [0..1[ representing a day
	...
	/**
	 * @param time a time (hh:mm:ss) in the interval 00:00:00 - 23:59:59
	...
	public static Double convertDateToDoubleTime(Date time) {
		...
		Calendar c = Calendar.getInstance();
		c.setTime(time);
		double t = ((c.get(Calendar.HOUR_OF_DAY) * 60 + c.get(Calendar.MINUTE)) * 60 + c.get(Calendar.SECOND)) * 1000
	...
	/**
	 * Calculates the number of days between <code>start</code> and
	 * <code>end</code>. If the end date is before the start date, the result will
	 * be positive as well. Example:
	 * <ul>
